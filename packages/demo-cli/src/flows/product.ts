...
export const executeProductFlow = async (c: OpenKartaClient): Promise<TranscriptStep[]> => {
  ...
  const search = (await c.search({ type: "product" })) as SearchRes;
  const itemId = search.items[0]!.id;
  steps.push({
    step: 2,
    action: "search",
    summary: `${search.items.length} products found`,
    detail: { firstItemId: itemId, firstTitle: search.items[0]!.title },
  });

  const cart = {
    cartId: `c_${Math.random().toString(36).slice(2, 8)}`,
    lines: [{ itemType: "product" as const, itemId, quantity: 1 }],
  };
...
export const runProductFlow = async (target: string): Promise<void> => {
  const c = createClient({ baseUrl: target });
  const steps = await executeProductFlow(c);
  printTranscript(`Product flow → ${target}`, steps);
};
