import { AgentPatchSchema, RegistryError, verificationToken } from "@openkarta/registry-shared";
...
export function agentsUpdateRouter() {
  ...
  router.patch("/agents/:id", requireSession, async (c) => {
    ...
    if (p.displayName !== undefined) {
      ...
      binds.push(p.displayName);
    }
    if (p.description !== undefined) {
      sets.push("description = ?");
      binds.push(p.description);
    }
    if (p.tier !== undefined) {
      sets.push("tier = ?");
      binds.push(p.tier);
    }
    if (p.supportedItemTypes !== undefined) {
      sets.push("supported_item_types = ?");
      binds.push(JSON.stringify(p.supportedItemTypes));
    }
    if (p.regions !== undefined) {
      sets.push("regions = ?");
      binds.push(JSON.stringify(p.regions));
    }
    if (p.tags !== undefined) {
      sets.push("tags = ?");
      binds.push(JSON.stringify(p.tags));
    }
    if (p.publicKey !== undefined) {
      sets.push("public_key = ?");
      binds.push(p.publicKey);
    }
    if (p.manifestUrl !== undefined) {
      sets.push("manifest_url = ?");
      binds.push(p.manifestUrl);
    }
    ...
    if (sets.length > 1) {
      await c.env.DB.prepare(`UPDATE agents SET ${sets.join(", ")} WHERE id = ?`)
        .bind(...binds)
        .run();
    }
    if (baseUrlChanged) {
