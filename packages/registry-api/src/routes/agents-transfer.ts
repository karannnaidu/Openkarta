import { type EmailClient, RegistryError, ulid } from "@openkarta/registry-shared";
...
export function agentsTransferRouter(getEmailClient: (env: Bindings) => EmailClient) {
  ...
  router.get("/agents/transfer/accept", requireSession, async (c) => {
    ...
    if (invite.to_email !== account.email) {
      return c.json(
        new RegistryError("forbidden", "invite addressed to another email").toJSON(),
        403,
      );
    }
    await c.env.DB.batch([
      ...
      c.env.DB.prepare("UPDATE transfer_invites SET consumed_at = ? WHERE token = ?").bind(
        now,
        tok,
      ),
    ]);
