...
const stubClient: EmailClient = {
  ...
  async sendVerificationPassed() {
    return { id: "stub" };
  },
  async sendHealthTransition() {
    return { id: "stub" };
  },
  async sendTransferInvite(args) {
    sentTransfer.push(args);
    return { id: "stub" };
  },
};
...
describe("POST /v1/agents/:id/verify", () => {
    ...
    const res = await app.request(
      "http://x/v1/agents",
      {
        ...
        body: JSON.stringify({
          ...VALID_AGENT,
          agentId,
          baseUrl: `https://${agentId}.example.com`,
          tier,
        }),
      },
      env,
  ...
  it("passes when well-known returns the matching token; flips verification_status", async () => {
    ...
    fetchMock
      ...
      .reply(200, token);
    const res = await app.request(
      "http://x/v1/agents/verify-pass/verify",
      {
        method: "POST",
        headers: { cookie },
      },
    ...
    const row = await env.DB.prepare(
      "SELECT verification_status FROM agents WHERE id = 'verify-pass'",
  ...
  it("fails (409) when token mismatches; status remains pending", async () => {
    ...
    const res = await app.request(
      "http://x/v1/agents/verify-mismatch/verify",
      {
        method: "POST",
        headers: { cookie },
      },
    ...
    expect(res.status).toBe(409);
    const row = await env.DB.prepare(
      "SELECT verification_status FROM agents WHERE id = 'verify-mismatch'",
    ).first<{ verification_status: string }>();
  ...
  it("lite tier auto-passes without HTTP fetch", async () => {
    ...
    const res = await app.request(
      "http://x/v1/agents/verify-lite/verify",
      {
        method: "POST",
        headers: { cookie },
      },
  ...
  it("non-owner gets 403", async () => {
    ...
    const res = await app.request(
      "http://x/v1/agents/owned-by-someone-else/verify",
      {
        method: "POST",
...
describe("Public reads", () => {
  beforeAll(async () => {
    ...
    await env.DB.batch([
      insert("pub-a", "healthy"),
      insert("pub-b", "stale"),
      insert("pub-c", "delisted"),
    ]);
...
describe("PATCH /v1/agents/:id", () => {
  it("changing baseUrl resets verification to pending and issues a new token", async () => {
    ...
    await env.DB.prepare(
      "UPDATE agents SET verification_status='verified' WHERE id='patchme'",
    ).run();
    const res = await app.request(
      "http://x/v1/agents/patchme",
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json", cookie },
    ...
    expect(res.status).toBe(200);
    const row = await env.DB.prepare(
      "SELECT verification_status, base_url FROM agents WHERE id='patchme'",
    ).first<{ verification_status: string; base_url: string }>();
    expect(row?.verification_status).toBe("pending");
    expect(row?.base_url).toBe("https://new.example.com");
    const pending = await env.DB.prepare(
      "SELECT count(*) as n FROM verifications WHERE agent_id='patchme' AND status='pending'",
    ).first<{ n: number }>();
...
describe("POST /v1/agents/:id/reverify-conformance", () => {
  it("rate-limits second call within 1hr", async () => {
    ...
    await env.DB.prepare(
      "UPDATE agents SET verification_status='verified' WHERE id='rev-target'",
    ).run();
    const r1 = await app.request(
      "http://x/v1/agents/rev-target/reverify-conformance",
...
describe("Transfer flow", () => {
  it("owner invites + recipient accepts", async () => {
    ...
    const inviteRes = await app.request(
      "http://x/v1/agents/xfer-target/transfer",
      {
        method: "POST",
        headers: { "Content-Type": "application/json", cookie: ownerCookie },
    ...
    const accept = await app.request(
      `http://x/v1/agents/transfer/accept?token=${tok}`,
      { headers: { cookie: recipientCookie } },
      env,
    );
    expect(accept.status).toBe(200);
    const newOwner = await env.DB.prepare(
      "SELECT account_id FROM agents WHERE id='xfer-target'",
    ).first<{ account_id: string }>();
    const recipient = await env.DB.prepare(
      "SELECT id FROM accounts WHERE email='xfer-to@example.com'",
    ).first<{ id: string }>();
