...
const stubClient: EmailClient = {
  ...
  async sendVerificationPassed({ to }) {
    sent.push({ kind: "verification_passed", to });
    return { id: "stub" };
  },
  async sendHealthTransition({ to }) {
    sent.push({ kind: "health_transition", to });
    return { id: "stub" };
  },
  async sendTransferInvite({ to }) {
    sent.push({ kind: "transfer_invite", to });
    return { id: "stub" };
  },
};
...
describe("POST /auth/magic-link", () => {
  it("returns 204 + writes magic_links row + sends email", async () => {
    ...
    const row = await env.DB.prepare("SELECT email, consumed_at FROM magic_links").first<{
      email: string;
  ...
  it("same response shape whether account exists or not (anti-enumeration)", async () => {
    ...
    const r2 = await app.request(
      "http://x/auth/magic-link",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: "still-unknown@example.com" }),
      },
      env,
    );
