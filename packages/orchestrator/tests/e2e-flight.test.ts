import { describe, it, expect, beforeAll, afterAll } from 'vitest';
import { mkdtempSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import type { FastifyInstance } from 'fastify';
import { loadFixtures, makeHandlers } from '@openkarta/reference-agent-travel/dist/agent.js';
import { createServer } from '@openkarta/sdk-node';
import { createOrchestrator, newCart, addLine, quoteCart, checkoutCart, createOrderStore } from '../src/index.js';

let url: string;
let app: FastifyInstance;
let tmp: string;

beforeAll(async () => {
  const fx = loadFixtures('fixtures');
  app = createServer({ handlers: makeHandlers(fx, 'test-secret-32-bytes-________'), secret: 'test-secret-32-bytes-________' });
  url = await app.listen({ port: 0, host: '127.0.0.1' });
  tmp = mkdtempSync(join(tmpdir(), 'okt-flight-'));
});
afterAll(async () => { await app.close(); rmSync(tmp, { recursive: true, force: true }); });

describe('orchestrator e2e (flight)', () => {
  it('search → cart → quote → checkout', async () => {
    const orch = createOrchestrator({
      registry: { version: '0.1', updated: '2026-04-24', agents: [
        { agentId: 'travel', displayName: 'Travel', baseUrl: url, tier: 'http',
          supportedItemTypes: ['flight'], regions: [{ country: 'IN' }], addedAt: '2026-04-24' },
      ]},
    });
    const results = await orch.search({
      itemType: 'flight',
      extra: { origin: 'BLR', destination: 'BOM', departure: '2026-05-10', pax: 1 },
    });
    expect(results.length).toBeGreaterThan(0);
    const top = results[0]!;

    let cart = newCart({ agentId: top.agentId, agentBaseUrl: url, itemType: 'flight', currency: 'INR' });
    cart = addLine(cart, {
      itemId: top.item.id, quantity: 1,
      extra: { passengers: [{ firstName: 'Karan', lastName: 'Naidu', gender: 'M' }] },
    });
    const quote = await quoteCart(cart);
    const store = createOrderStore({ ordersFile: join(tmp, 'orders.json') });
    const order = await checkoutCart({ cart, quote, payment: { method: 'cod' }, store });
    expect(order.orderId).toBeTruthy();
  });
});
