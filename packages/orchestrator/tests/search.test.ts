...
describe('searchAcrossAgents', () => {
  it('aggregates results across multiple agents', async () => {
    const agents: RegistryAgent[] = [
      {
        agentId: 'shop1', displayName: 'Shop 1', baseUrl: url1,
        tier: 'http', supportedItemTypes: ['product'], addedAt: '2024-01-01',
      },
      {
        agentId: 'shop2', displayName: 'Shop 2', baseUrl: url2,
        tier: 'http', supportedItemTypes: ['product'], addedAt: '2024-01-01',
      },
    ];
  ...
  it('continues past a failing agent', async () => {
    const agents: RegistryAgent[] = [
      ...
      {
        agentId: 'dead', displayName: 'Dead Agent', baseUrl: 'http://127.0.0.1:1',
        tier: 'http', supportedItemTypes: ['product'], addedAt: '2024-01-01',
      },
    ];
    const cache = createManifestCache({ ttlMs: 30_000 });
