...
const test = async (name: string, fn: () => Promise<string | void>): Promise<TestResult> => {
  ...
  try {
    const note = await fn();
    return {
      name, pack: 'flight', passed: true, durationMs: Date.now() - t0,
      ...(note ? { message: note } : {}),
    };
  } catch (e) {
    return { name, pack: 'flight', passed: false, message: String((e as Error).message), durationMs: Date.now() - t0 };
  }
};
...
interface FlightSearchItem { id: string; refundable?: boolean; }
...
export const runFlightPack = async (ctx: RunCtx): Promise<PackReport> => {
  ...
  const search = await fetchJson<{ items: FlightSearchItem[] }>(`${ctx.baseUrl}/v0/search`, {
    ...
    body: JSON.stringify({ query: { type: 'flight', origin: 'BLR', destination: 'BOM', departure: '2026-12-01', pax: 1 } }),
  });
  const flights = search.items ?? [];

  // Test 1: passenger-validation — sdk-node validates schemas (firstName required)
  results.push(await test('passenger-validation', async () => {
  ...
  results.push(await test('seat-selection', async () => {
    const flight = flights[0];
    if (!flight) return 'skipped: no flight items';
    const cart = {
      cartId: 'conformance_seat',
      lines: [{
        itemType: 'flight', itemId: flight.id,
    ...
    const bad = await fetch(`${ctx.baseUrl}/v0/quote`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ cart: { ...cart, cartId: 'conformance_seat_bad', lines: [{ ...cart.lines[0], seatSelection: ['99Z'] }] } }),
    });
  ...
  results.push(await test('pnr-issuance', async () => {
    const flight = flights[0];
    if (!flight) return 'skipped: no flight items';
    const cart = {
      cartId: 'conformance_pnr',
      lines: [{ itemType: 'flight', itemId: flight.id, passengers: [{ firstName: 'A', lastName: 'B' }] }],
    };
    ...
    if (!order.trackingRef) return 'warn: order missing trackingRef (PNR); harness does not issue';
  }));

  // Test 4: fare-rules-refund — soft
  results.push(await test('fare-rules-refund', async () => {
    const refundable = flights.find((f) => f.refundable === true);
    const nonRefund  = flights.find((f) => f.refundable === false);
    if (!refundable || !nonRefund) return 'skipped: catalogue lacks both refundable and non-refundable flights';
    return 'warn: harness flat-refunds 0 regardless of fare class';
  }));

  // Test 5: multi-leg-rollback — soft
  results.push(await test('multi-leg-rollback', async () => {
  ...
  return { pack: 'flight', tests: results, passedCount, failedCount,
