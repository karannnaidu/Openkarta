...
interface QuoteShape {
  quoteToken: string;
  estimatedFulfilmentAt?: string;
}
interface CheckoutResp {
  orderId: string;
}
interface ReturnResp {
  status?: string;
}
...
export const runProductPack = async (ctx: RunCtx): Promise<PackReport> => {
  ...
  results.push(
    await test("variants", async () => {
      ...
      const cart = {
        cartId: "conformance_variants",
        lines: [{ itemType: "product", itemId: withVariants.id, quantity: 1, variantSku }],
      };
  ...
  results.push(
    await test("delivery-modes", async () => {
      ...
      if (!q.estimatedFulfilmentAt)
        throw new Error("instant delivery declared but no estimatedFulfilmentAt");
      const eta = new Date(q.estimatedFulfilmentAt).getTime();
      const ceiling = Date.now() + 45 * 60_000;
      if (eta > ceiling)
        throw new Error(`instant ETA ${q.estimatedFulfilmentAt} > 45 min from now`);
    }),
  );

  // Test 3: inventory-states — SHOULD not MUST; warn if not enforced
  results.push(
    await test("inventory-states", async () => {
      ...
      const cart = {
        cartId: "conformance_inventory",
        lines: [{ itemType: "product", itemId: out.id, quantity: 1 }],
      };
      ...
      if (res.ok)
        return "warn: agent does not enforce inventory_unavailable on out items (SHOULD-not-MUST)";
  ...
  results.push(
    await test("return-window", async () => {
      const window = manifest.productCapabilities?.returnWindow;
      if (window === undefined)
        throw new Error("manifest missing productCapabilities.returnWindow");
      const itemId = items[0]?.id;
      if (!itemId) throw new Error("no product to checkout");
      const cart = {
        cartId: "conformance_return",
        lines: [{ itemType: "product", itemId, quantity: 1 }],
      };
      const q = await fetchJson<QuoteShape>(`${ctx.baseUrl}/v0/quote`, {
        method: "POST",
      ...
      const refund = await fetchJson<ReturnResp>(
        `${ctx.baseUrl}/v0/orders/${order.orderId}/return`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
      ...
      if (refund.status !== "initiated")
        throw new Error(`expected refund.status=initiated, got ${String(refund.status)}`);
    }),
  );
