...
const test = async (name: string, fn: () => Promise<string | void>): Promise<TestResult> => {
  ...
  try {
    const note = await fn();
    return {
      name, pack: 'bus', passed: true, durationMs: Date.now() - t0,
      ...(note ? { message: note } : {}),
    };
  } catch (e) {
    return { name, pack: 'bus', passed: false, message: String((e as Error).message), durationMs: Date.now() - t0 };
  }
};
...
export const runBusPack = async (ctx: RunCtx): Promise<PackReport> => {
  ...
  const search = await fetchJson<{ items: BusSearchItem[] }>(`${ctx.baseUrl}/v0/search`, {
    ...
    body: JSON.stringify({ query: { type: 'bus', origin: 'Bengaluru', destination: 'Pune', departure: '2026-12-01', pax: 1 } }),
  });
  const buses = search.items ?? [];

  // Test 1: boarding-point-selection — soft (harness doesn't cross-validate boarding IDs)
  results.push(await test('boarding-point-selection', async () => {
    const bus = buses[0];
    if (!bus) return 'skipped: no bus items in catalogue';
    const cart = {
      cartId: 'conformance_bp',
      lines: [{
        itemType: 'bus', itemId: bus.id,
    ...
    if (res.ok) return 'warn: agent does not validate boardingPointId against item (SHOULD return 422)';
  }));
  ...
  results.push(await test('operator-cancellation', async () => {
    const bus = buses[0];
    ...
    const cancelled = await fetchJson<{ fulfilmentStatus?: { state?: string } }>(`${ctx.baseUrl}/v0/orders/${order.orderId}/cancel`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ reason: 'operator_cancelled' }),
    });
    if (cancelled.fulfilmentStatus?.state !== 'cancelled') {
      throw new Error(`expected fulfilmentStatus.state=cancelled, got ${String(cancelled.fulfilmentStatus?.state)}`);
    }
  }));

  // Test 4: travel-date-validation — soft (sdk-node Cart has no past-date check)
  results.push(await test('travel-date-validation', async () => {
    return 'warn: bus cart schema does not capture travel date directly; agent enforcement out of scope';
  }));

  const passedCount = results.filter((r) => r.passed).length;
  const failedCount = results.length - passedCount;
  return { pack: 'bus', tests: results, passedCount, failedCount,
