...
const test = async (name: string, fn: () => Promise<string | void>): Promise<TestResult> => {
  ...
  try {
    const note = await fn();
    return {
      name, pack: 'stay', passed: true, durationMs: Date.now() - t0,
      ...(note ? { message: note } : {}),
    };
  } catch (e) {
    return { name, pack: 'stay', passed: false, message: String((e as Error).message), durationMs: Date.now() - t0 };
  }
};
...
export const runStayPack = async (ctx: RunCtx): Promise<PackReport> => {
  ...
  results.push(await test('availability-calendar', async () => {
    const stay = stays[0];
    if (!stay) throw new Error('no stay items to probe');
    const cart = { cartId: 'conformance_avail', lines: [{ itemType: 'stay', itemId: stay.id, checkIn: '2026-12-01', checkOut: '2026-12-02', guests: 2 }] };
  ...
  results.push(await test('cancellation-policy-refund', async () => {
    ...
    const cart = { cartId: 'conformance_cancel', lines: [{ itemType: 'stay', itemId: probe.id, checkIn: '2026-12-10', checkOut: '2026-12-11', guests: 2 }] };
    const q = await fetchJson<{ quoteToken: string }>(`${ctx.baseUrl}/v0/quote`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ cart }),
    });
    ...
    const cancel = await fetchJson<{ fulfilmentStatus?: { state?: string } }>(`${ctx.baseUrl}/v0/orders/${order.orderId}/cancel`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
  ...
  results.push(await test('checkin-checkout-times', async () => {
  ...
  results.push(await test('service-add-on', async () => {
    const stay = stays[0];
    if (!stay) throw new Error('no stay item to probe');
    const cart = {
      cartId: 'conformance_addon',
      lines: [
        { itemType: 'stay', itemId: stay.id, checkIn: '2026-12-01', checkOut: '2026-12-02', guests: 2 },
        { itemType: 'service', itemId: 'sv_xyz', slotStart: '2026-12-01T10:00:00Z', slotEnd: '2026-12-01T11:00:00Z', headcount: 1 },
      ],
    };
  ...
  results.push(await test('multi-night-pricing', async () => {
    const stay = stays[0];
    if (!stay) throw new Error('no stay item to probe');
    const cart = { cartId: 'conformance_nights', lines: [{ itemType: 'stay', itemId: stay.id, checkIn: '2026-12-01', checkOut: '2026-12-04', guests: 2 }] };
    ...
    if (typeof stay.priceMinor !== 'number') return 'warn: item lacks priceMinor; cannot verify multi-night math';
  ...
  return { pack: 'stay', tests: results, passedCount, failedCount,
