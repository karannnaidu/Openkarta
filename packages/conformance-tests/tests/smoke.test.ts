import {
  bootAgent as bootShop,
  loadFixtures as fxShop,
} from "@openkarta/reference-agent-shop/src/agent";
import {
  bootAgent as bootStays,
  loadFixtures as fxStays,
} from "@openkarta/reference-agent-stays/src/agent";
import {
  bootAgent as bootTravel,
  loadFixtures as fxTravel,
} from "@openkarta/reference-agent-travel/src/agent";
...
describe("smoke against all 3 reference agents", () => {
  it("Halcyon Shop: core+product, all pass", async () => {
    ...
    const failed = packReports.flatMap((r) =>
      r.tests.filter((t) => !t.passed).map((t) => `${r.pack}/${t.name}: ${t.message}`),
    );
