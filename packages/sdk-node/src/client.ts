...
export const createClient = (opts: ClientOptions): OpenKartaClient => ({
  ...
  search: (query) =>
    doFetch<SearchResults>(opts, "/v0/search", {
      method: "POST",
      body: JSON.stringify({ query }),
    }),
  get: (itemId) => doFetch<ItemBase>(opts, `/v0/items/${encodeURIComponent(itemId)}`),
  quote: (cart, userContext) =>
    doFetch<Quote>(opts, "/v0/quote", {
      method: "POST",
      body: JSON.stringify({ cart, userContext }),
    }),
  checkout: (input) =>
    doFetch<Order>(opts, "/v0/checkout", {
      method: "POST",
      body: JSON.stringify(input),
    }),
  status: (orderId) => doFetch<Order>(opts, `/v0/orders/${encodeURIComponent(orderId)}/status`),
  cancel: (orderId, reason) =>
    doFetch<Order>(opts, `/v0/orders/${encodeURIComponent(orderId)}/cancel`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    }),
  return: (orderId, input) =>
    doFetch<Refund>(opts, `/v0/orders/${encodeURIComponent(orderId)}/return`, {
      method: "POST",
