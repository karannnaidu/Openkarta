...
import { buildMirrorPayload, gitMirrorSnapshot, type MirrorPayload } from '../src/git-mirror.js';
...
describe('gitMirrorSnapshot()', () => {
  it('makes the expected GitHub REST sequence and lands a new commit', async () => {
    ...
    fetchMock
      ...
      .reply(200, (req) => {
        calls.push({ method: 'PATCH', path: '/git/refs/heads/registry-mirror', body: JSON.parse(req.body as string) });
    ...
    const treeBody = treeCall.body as { base_tree: string; tree: Array<{ path: string; mode: string; sha: string }> };
